function payTrial(auxVars,posSure,accepted,rejected,task)
...
if strcmp(task,'MG')
    xRisk  = [accepted(1)+rejected(1),accepted(2)+rejected(2)];
    cSure = {centerSure-w*[-4;-4],centerSure-w*[+4;-4],...
             centerSure-w*[+4;+4],centerSure-w*[-4;+4]}; % Centers for sure offer
    ...
    cG = {centerGam-w*[-4;-4],centerGam-w*[+4;-4],...
          centerGam-w*[+4;+4],centerGam-w*[-4;+4]}; % Centers for gambling offer
    locGam = randperm(4,2);
    for k = 1:2
        i = locGam(k);
    ...
    risk  = accepted(2) * rejected(2);
    cSure = {centerSure,centerSure-w*[-4;-4],centerSure-w*[+4;-4],...
                        centerSure-w*[+4;+4],centerSure-w*[-4;+4]}; % Centers for sure offer
    cG = {centerGam,centerGam-w*[-7;-7],centerGam-w*[+7;-7],...
                    centerGam-w*[+7;+7],centerGam-w*[-7;+7]}; % Centers for gambling offer
    locSure = randi(5);
    sS = repmat(cSure{locSure},1,8) + sBox;
    sSRect = [sS(:,1)',sS(:,4)'];
...
Screen('Flip',wd,[],1);
WaitSecs(fbTime);
if accepted(1)*accepted(2) ~= accepted(1) % Gambled: put a blue box feedback, empty the others
